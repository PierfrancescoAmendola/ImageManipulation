...
if __name__ == '__main__':
    ...
    # 3. SETUP DEVICE (GPU/CPU) - CORRETTO
    ...
    transform = transforms.Compose([
        transforms.Resize(224),  # <-- AGGIUNTA FONDAMENTALE PER ALEXNET
        transforms.ToTensor(),
    ...
    writer = SummaryWriter('runs/SuiteTest_1_AlexNet/' + str_log,comment=f"{str_log} batch_{batch_size} lr_{lr} epoch{epochNumber} Fold_{fold}")

    print("Caricamento modello AlexNet...")
    net = torchvision.models.alexnet(weights='IMAGENET1K_V1')

    # Modifica dell'ultimo layer per CIFAR-10 (10 classi) per AlexNet
    # L'ultimo strato di AlexNet è il sesto elemento (indice 6) nel blocco 'classifier'
    num_ftrs = net.classifier[6].in_features
    net.classifier[6] = nn.Linear(num_ftrs, 10)

    # Sposta la rete sul device (GPU o CPU) - FONDAMENTALE FARLO QUI
    net = net.to(device)
